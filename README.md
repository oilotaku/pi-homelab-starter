...
- PIN 只有 6 位數字,keyspace 遠小於原本考慮過的長 token,單純靠長度不夠安全,所以伺服器端加了**依來源 IP 分別計算**的失敗鎖定(連續錯 5 次鎖定,鎖定時間隨連續觸發次數倍增,5 分鐘起跳、上限 1 小時)。依 IP 分開鎖定是刻意的設計——第一版曾經做成全域鎖定,任何人送 5 次錯的 PIN 就能連帶把正常使用者也鎖在外面,等於免費的 DoS,已經修正
